...
class SCAT(object):
    """Standard SCAT network for image restoration and enhancement.

    Uses a convolutional neural network created by :func:`scatnet.internals.nets.common_unet`.
    

    Parameters
    ----------
    config : :class:`csbdeep.models.Config` or None
        Valid configuration of SCAT network (see :func:`Config.is_valid`).
    ...
    Example
    -------
    >>> model = SCAT(config, 'my_model')

    Attributes
    ----------
    config : :class:`csbdeep.models.Config`
        Configuration of SCAT network, as provided during instantiation.
    ...
    def _find_and_load_weights(self,prefer='best'):
        ...
        weights_chosen = 'weights_now.h5'
        print("Loading network weights from '%s'." % weights_chosen)
        self.load_weights(weights_chosen)
    ...
    def prepare_for_training(self, optimizer=None, **kwargs):
        ...
        if self.basedir is not None:
            if self.config.train_checkpoint is not None:
                ...
                self.callbacks.append(ModelCheckpoint(str(self.logdir / 'weights.{epoch:02d}-{val_loss:.2f}.hdf5'), save_best_only=False,  save_weights_only=True,period=10))
